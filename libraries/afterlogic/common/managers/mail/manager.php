...
//							}
	...
	private function specialIndexSearch($oImapClient, $fItemCallback, $sFolderFullNameRaw)
	{
		...
		if (0 < $iCount)
		{
			$aResult = $this->specialSubRequest($oImapClient, '1:*', $fItemCallback, false);
			if (is_array($aResult))
			{
	...
	public function MessageList($oAccount, $sFolderFullNameRaw, $iOffset = 0, $iLimit = 20, $sSearch = '', $bUseThreads = false, $aFilters = array())
	{
		...
		if (0 < $iRealMessageCount)
		{
			...
			if (0 < strlen($sSearch) || 0 < count($aFilters))
			{
				...
				if (\preg_match('/has[ ]?:[ ]?attachments/i', $sSearch) ||
					\preg_match('/attach:([^\s]+)/i', $sSearch, $aMatch))
				{
					...
					\CApi::LogObject(array($sAttachmentName, $sAttachmentRegs));
				...
				{
					$bSearch = true;
					$sSearchCriterias = $this->getSearchBuilder($oImapClient, $sCutedSearch,
						$oAccount->GetDefaultTimeOffset() * 60, $aFilters)->Complete();
