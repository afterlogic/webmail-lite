...
class Actions extends ActionsBase
{
	...
	public function AjaxFetcherCreate()
	{
		...
		if ($bResult)
		{
			$sStartScript = '/opt/afterlogic/scripts/mailfetch-on-create.sh';
...
//				$oCapability->allowClientIncoming('TwilioAftId_'.$oAccount->IdTenant.'_'.$oAccount->User->TwilioNumber);
	...
	public function AjaxContactList()
	{
		...
		if ($this->oApiCapability->IsPersonalContactsSupported($oAccount))
		{
			if ('' === $sGroupId)
			{
				$iCount = $oApiContacts->GetContactItemsCount($oAccount->IdUser, $sSearch, $sFirstCharacter, 0, $iTenantId);
			}
			else
			{
				$iCount = $oApiContacts->GetContactItemsCount($oAccount->IdUser, $sSearch, $sFirstCharacter, $sGroupId);
				$iCount = $iCount + $oApiContacts->GetContactItemsCount($oAccount->IdUser, $sSearch, $sFirstCharacter, $sGroupId, $oAccount->IdTenant);
			}
			...
			if (0 < $iCount)
			{
				...
				if (is_array($aContacts))
				{
					if (('' !== $sGroupId))
					{
						$aContacts2 = $oApiContacts->GetContactItems(
							$oAccount->IdUser, $iSortField, $iSortOrder, $iOffset,
							$iLimit, $sSearch, $sFirstCharacter, $sGroupId, $oAccount->IdTenant);
						$aContacts = array_merge($aContacts, $aContacts2);
					}
