...
(function ($, window) {
		...
		this.$editableArea.on('paste', function (oEvent) {
			...
			if (oEvent.clipboardData)
			{
				...
				else if (sHtml !== '')
				{
					...
					sHtml = sHtml.replace(/[\r\n]/g, ''); //fix for inserting single-line HTML text into the middle of the line (lost the second part of the line)
	...
	Crea.prototype.changeSignatureContent = function (sSignatureContent)
	{
		...
		else
		{
			...
			if ($anchor.length > 0)
			{
				$($anchor[0]).before($('<br /><div data-anchor="signature">' + sSignatureContent + '</div><br />'));
			}
			else
			{
				...
				{
					this.$editableArea.append($('<br /><div data-anchor="signature">' + sSignatureContent + '</div><br />'));
				}
			}
		}
	};
...
//		this.execCom('Redo');
	...
	Crea.prototype.checkSignatureNode = function ()
	{
		if (window.getSelection)
		{
			var
				oSel = window.getSelection(),
				$Signature = $(oSel.anchorNode).closest('div[data-anchor=\'signature\']')
			;
			
			if ($Signature.length > 0)
			{
				$Signature = this.$editableArea.find('div[data-anchor=\'signature\']');
				$Signature.css({
	...
	Crea.prototype.clearSignatureLights = function ()
	{
		var $Signature = this.$editableArea.find('div[data-anchor=\'signature\']');
		$Signature.css({
			'border': 'none',
			'background': 'none'
		});
...
(function($) {
	...
	var inputosaurustext = {
		...
		_resizeInput : function(ev) {
			...
			{
				_.defer(function () {
					widget._resizeAnyPlaceInput(widget, 10);
				});
			}
			else
			{
