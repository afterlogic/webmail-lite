...
(function(){function a(a){function l(){if(g&&d<a){var b=g,c=b[0],f=Array.prototype.slice.call(b,1),m=b.index;g===h?g=h=null:g=g.next,++d,f.push(function(a,b){--d;if(i)return;a?e&&k(i=a,e=j=g=h=null):(j[m]=b,--e?l():k(null,j))}),c.apply(null,f)}}var c={},d=0,e=0,f=-1,g,h,i=null,j=[],k=b;return arguments.length<1&&(a=Infinity),c.defer=function(){if(!i){var a=arguments;a.index=++f,h?(h.next=a,h=h.next):g=h=a,++e,l()}return c},c.await=function(a){return k=a,e||k(i,j),c},c}function b(){}typeof module=="undefined"?self.queue=a:module.exports=a,a.version="0.0.2"})();(function ($, window, queue) {
	 ...
	 * @return {Array}
	 */
	function getDataFromFiles(aFiles)
	{
		var
			aResult = [],
			iSize = null,
		...
		aFiles = aFiles && 0 < aFiles.length ? aFiles : null;
		if (aFiles)
		{
			for (iIndex = 0, iLen = aFiles.length; iIndex < iLen; iIndex++)
			{
				oFile = aFiles[iIndex];
				if (oFile)
				{
					sFileName = isUndefined(oFile.fileName) ?
						(isUndefined(oFile.name) ? null : oFile.name) : oFile.fileName;
	
					iSize = isUndefined(oFile.fileSize) ?
						(isUndefined(oFile.size) ? null : oFile.size) : oFile.fileSize;
	
					sType = isUndefined(oFile.type) ? null : oFile.type;
	
					aResult.push([sFileName, iSize, sType, oFile]);
				}
			}
		}
	 ...
	 * @return {Array}
	 */
	function getDataFromInput(oInput)
	{
		var
			aResult = [],
			iSize = null,
			sType = null,
			sFileName = null,
			aFiles = oInput && oInput.files && 0 < oInput.files.length ? oInput.files : null,
			oFile = null
		;
	
		if (aFiles)
		{
			aResult = getDataFromFiles(aFiles);
		}
		else
		{
			sFileName = oInput.value.split('\\').pop().split('/').pop();
			aResult.push([sFileName, iSize, sType, oFile]);
		}
	 ...
	 * @return {Array}
	 */
	function getDataFromDragEvent(oEvent)
	{
		var
			aResult = [],
			aFiles = []
	...
	{
		return $('<input type="file" tabindex="-1" hidefocus="hidefocus" ' +
	'style="position: absolute; direction: ltr; right: 0px; top: 0px; font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer;" />').css({
		'opacity': 0
	});
	}
	 ...
	 * @return {string}
	 */
	function getStringOrCallFunction(mStringOrFunction)
	{
		return $.isFunction(mStringOrFunction) ? mStringOrFunction().toString() : mStringOrFunction.toString();
	}
	/**
	 * @constructor
	 * @param {Jua} oJua
	 ...
	 */
	AjaxDriver.prototype.uploadTask = function (sUid, oFile, fCallback)
	{
		...
		try
		{
			...
			oFormData.append(getValue(this.oOptions, 'name', 'juaFile'), oFile);
			$.each(aHidden, function (sKey, sValue) {
				oFormData.append(sKey, getStringOrCallFunction(sValue));
			});
	...
	AjaxDriver.prototype.generateNewInput = function (oClickElement)
	{
		...
		if (oClickElement)
		{
			...
			$(oClickElement).append(oInput);
	
			oInput
				.on('change', function () {
					var
						that = this,
						aInputData = getDataFromInput(this)
					;
	
					if (aInputData && 0 < aInputData.length)
					{
						self.oJua.addFiles(aInputData);
	 ...
	 * @param {Function} fCallback
	 */
	IframeDriver.prototype.uploadTask = function (sUid, oFile, fCallback)
	{
		if (false === this.oUids[sUid])
		{
			fCallback(null, sUid);
		...
		if (oForm)
		{
			...
			$.each(aHidden, function (sKey, sValue) {
				oForm.append($('<input type="hidden" />').attr('name', sKey).val(getStringOrCallFunction(sValue)));
			});
	...
	' style="position: absolute; right: 0px; top: 0px; margin: 0px; padding: 0px; cursor: pointer;"></form>').css({'opacity': 0});
	...
	' style="position: absolute; top: -1000px; left: -1000px; cursor: pointer;" />').css({'opacity': 0});
			...
			oForm.append(oInput).append(oIframe);
			...
			oInput
				.on('change', function () {
					var aInputData = getDataFromInput(this);
					if (aInputData && 1 === aInputData.length)
					{
						oForm.css({
							'position': 'absolute',
							'top': -1000,
						...
						self.oJua.addFile(sUid, aInputData[0]);
						self.generateNewInput(oClickElement);
					}
				})
			;
	...
	function Jua(oOptions)
	{
		...
		}, isUndefined(oOptions) ? {} : oOptions);
	
		this.oQueue = queue(pInt(getValue(this.oOptions, 'queueSize', 10)));
		if (getValue(this.oOptions, 'onCompleteAll', null))
		{
			this.oQueue.await(getValue(this.oOptions, 'onCompleteAll', Jua.fEmptyFunction));
		}
	
		this.oDriver = this.isAjaxUploaderSupported() && !getValue(this.oOptions, 'disableAjaxUpload', false) ?
			new AjaxDriver(this, this.oOptions) : new IframeDriver(this, this.oOptions);
	
		this.oClickElement = getValue(this.oOptions, 'clickElement', null);
	
		if (this.oClickElement)
		{
			$(this.oClickElement).css({
				'position': 'relative',
				'overflow': 'hidden',
			...
			!getValue(this.oOptions, 'disableAjaxUpload', false))
		{
			(function (self) {
				var
					...
					fHandleDrop = function (oEvent) {
						...
						if (oEvent)
						{
							...
							var aFiles = getDataFromDragEvent(oEvent);
							if (aFiles && 0 < aFiles.length)
							{
								getValue(self.oOptions, 'onDrop', Jua.fEmptyFunction)(oEvent, aFiles);
								getValue(self.oOptions, 'onDragLeave', Jua.fEmptyFunction)(oEvent, aFiles);
								self.addFiles(aFiles);
								clearTimeout(Jua.iDocTimer);
							}
						}
					},
					fHandleDragEnter = function (oEvent) {
						...
						if (oEvent)
						{
							...
							oEvent.preventDefault();
							getValue(self.oOptions, 'onDragEnter', Jua.fEmptyFunction)(oEvent, oDragAndDropElement);
						}
					},
					fHandleDragLeave = function (oEvent) {
						...
						if (oEvent)
						{
							...
							clearTimeout(Jua.iDocTimer);
							getValue(self.oOptions, 'onDragLeave', Jua.fEmptyFunction)(oDragAndDropElement);
							oEvent.stopPropagation();
						}
				...
				{
					Jua.aOptions.push(self.oOptions);
					if ('1' !== oBigDropZone.data('jua-out-drop'))
					{
						oBigDropZone.data('jua-out-drop', '1')
							.on('dragover', Jua.onBodyDragOver)
							.on('dragenter', Jua.onBodyDragEnter)
							.on('dragleave', Jua.onBodyDragLeave)
							.on('drop', Jua.onBodyDragDrop)
						;
					}
		...
		setValue(this, 'cancel', this.cancel);
		setValue(this, 'isDragAndDropSupported', this.isDragAndDropSupported);
		setValue(this, 'isAjaxUploaderSupported', this.isAjaxUploaderSupported);
	}
	 ...
	 */
	Jua.fireOnEvent = function (sName, aArguments)
	{
		var
			iIndex = 0,
			iLen = Jua.aOptions.length
		;
	
		for (; iIndex < iLen; iIndex++)
		{
			getValue(Jua.aOptions[iIndex], sName, Jua.fEmptyFunction).apply(aArguments);
		}
	};
	 ...
	 * @param {Event} oEvent
	 */
	Jua.onBodyDragOver = function (oEvent)
	{
		oEvent = getEvent(oEvent);
		if (oEvent)
		{
	 ...
	 * @param {Event} oEvent
	 */
	Jua.onBodyDragEnter = function (oEvent)
	{
		oEvent = getEvent(oEvent);
		if (oEvent)
		{
			...
			Jua.fireOnEvent('onBodyDragEnter', [oEvent]);
		}
	};
	 ...
	 * @param {Event} oEvent
	 */
	Jua.onBodyDragLeave = function (oEvent)
	{
		oEvent = getEvent(oEvent);
		if (oEvent)
		{
			Jua.iDocTimer = setTimeout(function () {
				Jua.fireOnEvent('onBodyDragLeave', []);
			}, 300);
		}
	};
	 ...
	 * @param {Event} oEvent
	 */
	Jua.onBodyDragDrop = function (oEvent)
	{
		oEvent = getEvent(oEvent);
		if (oEvent)
		{
	 ...
	 */
	Jua.prototype.addFile = function (sUid, aInputData)
	{
		var fOnSelect = getValue(this.oOptions, 'onSelect', null);
		if (!fOnSelect || (false !== fOnSelect(sUid, {
			'FileName': aInputData[0],
			'Size': aInputData[1],
			'Type': aInputData[2],
			'File' : aInputData[3]
		})))
		{
			...
			this.oQueue.defer(scopeBind(this.oDriver.uploadTask, this.oDriver), sUid, aInputData[3]);
		}
	};
	 ...
	 * @param {Array} aInputData
	 */
	Jua.prototype.addFiles = function (aInputData)
	{
		var
			iIndex = 0,
			iLen = aInputData.length
		;
	
		for (; iIndex < iLen; iIndex++)
		{
			this.addFile(getNewUid(), aInputData[iIndex]);
		}
	};
...
//		console.log(this.pane.inner.scrollLeft());
//		if (this.pane.inner.get(0).scrollWidth > this.pane.el.width()) {
//
//		}
	};
...
(function ($, window) {
	...
	Crea.prototype.initContentEditable = function ()
	{
		var
			...
			sDefaultFontSize = this.aPixelsSizes[iDefaultFontSize]
		;
	...
	/**
	 * Resizes rich editor area.
	 * 
	 * @param {string} sWidth
	 * @param {string} sHeight
	 */
	Crea.prototype.resize = function (sWidth, sHeight)
	{
		var
			iBodyPad = 10
		;
		
		this.oOptions.width = sWidth;
		this.oOptions.height = sHeight;
		
		if (this.$editableArea && this.$container)
		{
			this.$container.css({'width': sWidth, 'height': sHeight});
			this.$editableArea.css({'width': sWidth, 'height': sHeight});
	 ...
	 */
	Crea.prototype.getText = function ()
	{
		return this.$editableArea ? this.$editableArea.html() : '';
	};
	
	/**
	 * Sets focus.
	 */
	Crea.prototype.setFocus = function ()
	{
	...
	{
		var sCmd = ($.browser.msie && parseInt($.browser.version) < 9) ? 'CreateLink' : 'createlink';
	...
	{
		var sCmd = ($.browser.msie && parseInt($.browser.version) < 9) ? 'Unlink' : 'unlink';
		this.execCom(sCmd);
	};
	...
	{
		var sCmd = ($.browser.msie && parseInt($.browser.version) < 9) ? 'InsertImage' : 'insertimage';
		this.setFocus();
	...
	{
		var sCmd = ($.browser.msie) ? 'BackColor' : 'hilitecolor';
	...
	Crea.prototype.storeSelectionPosition = function ()
	{
		if (window.getSelection)
		{
			...
			for (; iIndex < iLen; ++iIndex)
			{
				aRanges.push(oSel.getRangeAt(iIndex));
			}
	...
	Crea.prototype.restoreSelectionPosition = function (sText)
	{
		...
		if (window.getSelection && $.isArray(this.aRanges))
		{
			...
			for (; iIndex < iLen; ++iIndex)
			{
				sRangeText += this.aRanges[iIndex];
				oSel.addRange(this.aRanges[iIndex]);
			}
