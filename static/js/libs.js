...
//		self.htmlarea.removeClass('editorDragOver');
	...
	Crea.prototype.getText = function (bRemoveSignatureAnchor)
	{
		...
		if (this.$editableArea)
		{
			...
			sVal = this.$editableArea ? this.$editableArea.html() : '';
...
(function($) {
	...
	var inputosaurustext = {
		...
		_create: function() {
			...
			els.input = $('<input type="text"' + tabindexStr + ' />');
		...
		_setChosen : function(valArr) {
			...
			$.each(valArr, function(k,v) {
				...
				if(v !== '' && (!exists || self.options.allowDuplicates)){
					...
					obj.value = v;
					obj.title = v.replace(/\s*<.*>/g, '').replace(/"/g, ''); 
		...
		// @name text for tag
		_createTag: function (name, key, fullValue) {
			var
				title = fullValue ? 
			...
			if (name !== undefined)
			{
				...
				li.draggable({
					stop: function(event, ui) {
						widget._removeLiTag($(ui.helper), widget);
					}
				});
			}
		...
		_renderTags : function() {
			...
			$.each(this._chosenValues, function(k,v) {
				var el = self._createTag(v.title, v.key, v.value);
