	...
	_.each(aParts, function (sPart) {
		if (sSubjectEnd.length === 0)
		{
			var
				...
				bRe = aRePrefixes.indexOf(sPartUpper) !== -1,
				bFwd = aFwdPrefixes.indexOf(sPartUpper) !== -1,
				iCount = 1,
			...
			if (!bRe && !bFwd)
			{
				...
				{
					bRe = aRePrefixes.indexOf(oMatch[1].toUpperCase()) !== -1;
					bFwd = aFwdPrefixes.indexOf(oMatch[1].toUpperCase()) !== -1;
...
Utils.Address.getFullEmail = function (sName, sEmail)
{
	...
	if (sEmail.length > 0)
	{
		if (sName.length > 0)
		{
			if (Utils.Address.isCorrectEmail(sName))
			{
				sFull = '"' + sName + '" <' + sEmail + '>';
			}
			else
...
Utils.Address.getArrayRecipients = function (sRecipients, bIncludeLastIncorrectEmail)
{
	...
	while (sEndRcp.length > 0)
	{
		...
		while (aSeparators.indexOf(sEndRcp[iNextPos + 1]) !== -1)
		{
			iNextPos++;
		}
 ...
 * @param {boolean=} bIgnoreSubscribed = false
 * @returns {Array}
 */
	...
	var
		...
		fGetOptionsFromCollection = function (aOrigCollection) {
			...
			for (iIndex = 0, iLen = aOrigCollection.length; iIndex < iLen; iIndex++)
			{
				...
				if (!oItem.bVirtual && (oItem.type() !== Enums.FolderTypes.Inbox && bHideInbox || !bHideInbox) && (!bIgnoreUnsubscribed || oItem.subscribed()))
				{
					aResCollection.push({
						'name': oItem.name(),
						'fullName': oItem.fullName(),
						'displayName': (new Array(oItem.iLevel + 1)).join(sDeepPrefix) + oItem.name(),
						'translatedDisplayName': (new Array(oItem.iLevel + 1)).join(sDeepPrefix) + oItem.displayName(),
						'disable': ((oItem.isSystem() && !bEnableSystem) || (!bIgnoreCanBeSelected && !oItem.bCanBeSelected))
					});
				}
