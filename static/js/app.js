													...
													oFetcherOrIdentity, sText, sDraftUid)
{
	...
	_.each(aAttachmentsLink, function (oAttachLink) {
		...
		{
			oReplyData.Attachments.push(oAttachLink.getCopy());
		}
	});
...
CAddressModel.prototype.mouseoverEvent = function (oAddress, oEvent)
{
	if (!this.oPopup || this.oPopup.length === 0)
	{
		...
		{
			this.oPopup
				.on('mouseover', _.bind(this.openPopup, this))
				.on('mouseout', _.bind(this.mouseoutEvent, this))
			;
		}
	}
...
CMessageListViewModel.prototype.routeForMessage = function (oMessage)
{
	...
	if (oMessage !== null)
	{
		...
		{
			this.changeRoutingForMessageList(sFolder, iPage, sUid, sSearch, this.filters());
			if (App.MailCache.currentMessage() && sUid === App.MailCache.currentMessage().uid())
			{
...
//			this.autoSaveTimer = window.setTimeout(fSave, 10 * 1000);
		}
	}
};
...
CContactsViewModel.prototype.isEnableSharing = function ()
{
	return 0 < this.selector.listCheckedOrSelected().length;
};
...
function CMailCache()
{
	...
	AppData.User.useThreads.subscribe(function () {
		...
		Utils.log('****************************************AppData.User.useThreads.subscribe');
		Utils.log(this.messages());
		Utils.log('****************************************');
		this.messages([]);
	}, this);
...
{
	Utils.log('****************************************showOpenedThreads');
	Utils.log(this.messages());
	Utils.log(sFolderFullName);
	Utils.log(this.uidList());
...
CMailCache.prototype.setMessagesFromUidList = function (oUidList, iOffset, oMessages, bFillMessages)
{
	...
	if (bFillMessages)
	{
		Utils.log('****************************************setMessagesFromUidList');
		Utils.log(this.messages());
		Utils.log(this.folderList().currentFolderFullName());
		Utils.log(oUidList);
		Utils.log(aMessages);
			...
			if (_.isArray(aData) && aData.length > 3)
			{
				...
				if (oFolderList)
				{
					...
					if (oFolder)
					{
						...
						if (bSameAccount && bFolderHasChanges && oFolder.fullName() === sCurrentFolderName)
						{
