...
function CMessageSender()
{
	...
	this.replyCc = ko.observable('');
	this.replyBcc = ko.observable('');
 ...
 * @param {string} sReplyCc
 * @param {string} sDraftBcc
 */
CMessageSender.prototype.setReplyData = function (sReplyText, sDraftUid, sReplyCc, sDraftBcc)
{
	this.replyText(sReplyText);
	this.replyDraftUid(sDraftUid);
	this.replyCc(sReplyCc || '');
	this.replyBcc(sDraftBcc || '');
};
...
CMessageSender.prototype.getSignatureText = function (iAccountId, oFetcherOrIdentity)
{
	...
	if (oFetcherOrIdentity && oFetcherOrIdentity.accountId() === iAccountId && oFetcherOrIdentity.signature() !== null)
	{
	...
	if (oAccount)
	{
		...
		if (!oFetcherOrIdentity)
		{
			oFetcherOrIdentity = _.find(oAccount.identities() || [], function (oIdnt) {
				var
					oAddress = _.find(aRecipients, function (oAddr) {
						return oAddr.sEmail === oIdnt.email();
					}),
					bHasAddress = !!oAddress
				;
				return bHasAddress;
 ...
 * @return {string}
 */
AfterLogicApi.i18n = function (sKey, oValueList, sDefaulValue, nPluralCount) {
	return Utils.i18n(sKey, oValueList, sDefaulValue, nPluralCount);
};
...
CPhone.prototype.incomingCall = function (sNumber)
{
	...
	if (sNumber)
	{
		App.Ajax.send(oParameters, function (oData) {
			...
			{
				_.each(oData.Result.List[0].Phones, function (sUserPhone, sKey) {
...
//		App.Api.showError(oError['message'], false, true);
	...
	this.device.incoming(function (oConnection) {
		...
		/*self.calls().push(
			{
				time: moment().hour() + ':' + moment().minute(),
				number: oConnection.parameters.From,
				direction: 'missed'
			}
		);*/
	});
...
//		console.log(self.connection.status());
//	}, 1000);
...
AccountCreatePopup.prototype.onEmailDomainDataResponse = function (oData, oParameters)
{
	this.loading(false);
	this.incomingMailLogin(this.email());
	...
	if (oData.Result)
	{
		this.incomingMailPort(oData.Result.IncomingMailPort);
		this.incomingMailServer(oData.Result.IncomingMailServer);
		this.outgoingMailPort(oData.Result.OutgoingMailPort);
		this.outgoingMailServer(oData.Result.OutgoingMailServer);
...
AccountCreatePopup.prototype.onAccountCreateResponse = function (oData, oParameters)
{
	this.loading(false);
...
FetcherAddPopup.prototype.onAddFetcherResponse = function (oResponse, oRequest)
{
	...
	if (!oResponse.Result)
	{
//		App.Api.showErrorByCode(oResponse.ErrorCode, Utils.i18n('WARNING/FETCHER_ADDING_ERROR'));
...
CalendarCreateEventPopup.prototype.getDisplayedTimes = function (sTimeFormatMoment)
{
	...
	{
		case 'HH:mm':
			aDisplayedTimes = [{'value':'00:00','text':'00:00'},{'value':'00:30','text':'00:30'},{'value':'01:00','text':'01:00'},{'value':'01:30','text':'01:30'},{'value':'02:00','text':'02:00'},{'value':'02:30','text':'02:30'},{'value':'03:00','text':'03:00'},{'value':'03:30','text':'03:30'},{'value':'04:00','text':'04:00'},{'value':'04:30','text':'04:30'},{'value':'05:00','text':'05:00'},{'value':'05:30','text':'05:30'},{'value':'06:00','text':'06:00'},{'value':'06:30','text':'06:30'},{'value':'07:00','text':'07:00'},{'value':'07:30','text':'07:30'},{'value':'08:00','text':'08:00'},{'value':'08:30','text':'08:30'},{'value':'09:00','text':'09:00'},{'value':'09:30','text':'09:30'},{'value':'10:00','text':'10:00'},{'value':'10:30','text':'10:30'},{'value':'11:00','text':'11:00'},{'value':'11:30','text':'11:30'},{'value':'12:00','text':'12:00'},{'value':'12:30','text':'12:30'},{'value':'13:00','text':'13:00'},{'value':'13:30','text':'13:30'},{'value':'14:00','text':'14:00'},{'value':'14:30','text':'14:30'},{'value':'15:00','text':'15:00'},{'value':'15:30','text':'15:30'},{'value':'16:00','text':'16:00'},{'value':'16:30','text':'16:30'},{'value':'17:00','text':'17:00'},{'value':'17:30','text':'17:30'},{'value':'18:00','text':'18:00'},{'value':'18:30','text':'18:30'},{'value':'19:00','text':'19:00'},{'value':'19:30','text':'19:30'},{'value':'20:00','text':'20:00'},{'value':'20:30','text':'20:30'},{'value':'21:00','text':'21:00'},{'value':'21:30','text':'21:30'},{'value':'22:00','text':'22:00'},{'value':'22:30','text':'22:30'},{'value':'23:00','text':'23:00'},{'value':'23:30','text':'23:30'}];
...
function CFolderModel()
{
	...
	App.MailCache.editedFolderList.subscribe(function (oFolderList) {
 ...
 * @param {boolean} bDefinitelyThreadPart
 * @param {boolean} bTrustThreadInfo
 */
CFolderModel.prototype.parseAndCacheMessage = function (oRawMessage, bDefinitelyThreadPart, bTrustThreadInfo)
{
	var
		sUid = oRawMessage.Uid.toString(),
		oMessage = this.oMessages[sUid] ? this.oMessages[sUid] : new CMessageModel(),
		bThreadPart = oMessage.threadPart() || bDefinitelyThreadPart
	;
...
CFolderModel.prototype.onMessageResponse = function (oData, oParameters)
{
	...
	{
		oMessage = this.parseAndCacheMessage(oResult, false, true);
...
CFolderModel.prototype.onAccordion = function ()
{
...
CMessageModel.prototype.downloadAllAttachments = function ()
{
	...
	{
		App.downloadByUrl(Utils.getDownloadLinkByHash(this.accountId(), this.allAttachmentsHash));
	}
	else
	{
		var
...
//	this.calendars = App.CalendarCache.calendars;
	this.calendars = ko.computed(function () {
		return _.filter(App.CalendarCache.calendars(), function(oCalendar){ return oCalendar.canEdit; });
	}, this);
...
CGroupModel.prototype.viewAllMails = function (aContactsList)
{
	var
//		aContactsList = App.Screens.oScreens.contacts.Model.selectedGroupContactsList(),
...
//		App.MailCache.searchMessagesInInbox('email:' + (_.uniq(_.without(aEmails, '')).join(',')));
	}
};
...
CCalendarModel.prototype.addEvent = function (oEvent)
{
	if (oEvent && !this.eventExists(oEvent.id))
	{
		...
		if (!this.isEditable())
		{
			oEvent.className = 'fc-event-readonly';
		}
...
//		this.action('offline_active');
	...
	else */if (this.action() === 'offline_active') {
		this.action('offline');
	}
...
//					sCategory = oItem.Name === '' ? oItem.Email : oItem.Name + ' ' + oItem.Email;
					sCategory = oItem.Name + ' ' + oItem.Email;
...
CMessageListViewModel.prototype.requestMessageList = function ()
{
	...
	else
	{
		this.isLoading(true);
		this.isError(false);
		App.MailCache.getFolderList(AppData.Accounts.currentId());
	}
};
...
function CMessagePaneViewModel()
{
	...
	this.visibleSaveAttachmentsToFiles = ko.observable(false);
	this.visibleDownloadAllAttachmentsSeparately = ko.observable(true);
	this.downloadMenuExpanded = ko.observable(false);
	...
	$(document).on('keyup', $.proxy(function(ev) {
		if (App.Screens.currentScreen() === Enums.Screens.Mailbox && ev && ev.keyCode === Enums.Key.q && !Utils.isTextFieldFocused()) {
...
{
	var sSearch = this.fromEmail();
	
	if (this.hasFromContact())
	{
		sSearch = this.fromContact().getEmailsString();
	}
...
CMessagePaneViewModel.prototype.onCurrentMessageSubscribe = function ()
{
	...
	if (oMessage && this.uid() === oMessage.uid())
	{
		...
		sFullTo = oMessage.oTo.getDisplay(Utils.i18n('MESSAGE/ME_RECIPIENT'), oAccount.email());
		sFullCc = oMessage.oCc.getDisplay(Utils.i18n('MESSAGE/ME_RECIPIENT'), oAccount.email());
		sFullBcc = oMessage.oBcc.getDisplay(Utils.i18n('MESSAGE/ME_RECIPIENT'), oAccount.email());
		if (sFullTo.length > 0)
		{
...
CMessagePaneViewModel.prototype.toggleDownloadMenu = function ()
{
	this.downloadMenuExpanded(!this.downloadMenuExpanded());
//	console.log(this.downloadMenuExpanded());
};
...
CMessagePaneViewModel.prototype.executeReplyOrResend = function (sReplyType)
{
	if (this.currentMessage())
	{
		App.MessageSender.setReplyData(this.replyHtmlText(), this.replyDraftUid(), this.replyCc(), this.replyBcc());
		this.replyText('');
		this.replyDraftUid('');
		this.replyCc('');
		this.replyBcc('');
...
CMessagePaneViewModel.prototype.executeSave = function ()
{
	if (this.currentMessage())
	{
		App.downloadByUrl(this.currentMessage().downloadLink());
	}
};
...
function CMailViewModel()
{
	...
	this.selectedUids = ko.computed(function () {
		var
			aChecked = this.oMessageList.selector.listChecked(),
			aThreadCheckedUids = this.getThreadCheckedUidsFromList(aChecked),
 ...
 * @param {Array} aMessages
 */
CMailViewModel.prototype.getThreadCheckedUidsFromList = function (aMessages)
{
	var
	...
	_.each(aMessages, function (oMessage) {
		if (oMessage.threadCount() > 0 && !oMessage.threadOpened())
		{
			_.each(oMessage.threadUids(), function (sUid) {
				...
				if (oThreadMessage && oThreadMessage.checked())
				{
...
CMailViewModel.prototype.dragAndDropHelper = function (oMessage)
{
	...
	var
		oSelectedItem = this.oMessageList.selector.itemSelected(),
		oHelper = Utils.draggableMessages(),
		aUids = this.selectedUids(),
		iCount = aUids.length
	;
	
	if (aUids.length === 0 && oSelectedItem)
	{
		aUids = [oSelectedItem.uid()];
		iCount = 1;
	}
...
//			this.autoSaveTimer = window.setTimeout(fSave, AppData.App.AutoSaveIntervalSeconds * 1000);
			this.autoSaveTimer = window.setTimeout(fSave, 15000);
		}
	}
};
...
CComposeViewModel.prototype.onMessageResponse = function (oMessage)
{
	...
	else
	{
		switch (this.routeType())
		{
			...
			case Enums.ReplyType.ReplyAll:
			case Enums.ReplyType.Resend:
				oFetcherOrIdentity = App.MessageSender.getFetcherOrIdentityByRecipients(oMessage.oTo.aCollection, oMessage.accountId());
				if (oFetcherOrIdentity)
				{
					this.selectedFetcherOrIdentity(oFetcherOrIdentity);
				}
...
CSettingsViewModel.prototype.onResponseFolderDelete = function (oResponse, oRequest)
{
	if (!oResponse.Result)
	{
		App.Api.showErrorByCode(oResponse.ErrorCode, Utils.i18n('SETTINGS/ERROR_SETTINGS_SAVING_FAILED'));
	}
	else
	{
		App.MailCache.getFolderList(AppData.Accounts.editedId());
	...
	else
	{
		...
		else
		{
			...
			AppData.User.updateCommonSettings(oParameters.MailsPerPage, oParameters.ContactsPerPage,
				...
				oParameters.DefaultTimeFormat, oParameters.UseThreads, oParameters.SaveRepliedMessagesToCurrentFolder);
...
CEmailAccountsSettingsViewModel.prototype.onFetcherDelete = function (oData)
{
	var
		...
		fCallBack = _.bind(this.fetcherDelete, this, oData.id()),
...
CIdentityPropertiesViewModel.prototype.onUpdateIdentityResponse = function (oResponse, oRequest)
{
	...
	if (!oResponse.Result)
	{
		App.Api.showErrorByCode(oResponse.ErrorCode, Utils.i18n('SETTINGS/ACCOUNTS_IDENTITY_ADDING_ERROR'));
	}
	else
	{
...
CIdentityPropertiesViewModel.prototype.onDeleteIdentityResponse = function (oResponse, oRequest)
{
	if (!oResponse.Result)
	{
		App.Api.showErrorByCode(oResponse.ErrorCode, Utils.i18n('SETTINGS/ACCOUNTS_IDENTITY_DELETING_ERROR'));
	}
	AppData.Accounts.populateIdentities();
};
...
{
	this.getEvents(this.calendars.ids());
};
...
//	console.log(this.publicCalendarId);
...
CCalendarViewModel.prototype.onCalendarsResponse = function (oData, oParameters)
{
	...
	if (oData.Result)
	{
		...
		this.calendars.clearCollection();
...
CFileStorageViewModel.prototype.onApplyBindings = function ($viewModel)
{
	...
	$(document).on('keyup', function(ev) {
		if (ev && ev.keyCode === Enums.Key.s && self.selector.useKeyboardKeys() && !Utils.isTextFieldFocused()) {
			self.isSearchFocused(true);
		}
	});	
};
...
{
	var sSearch = this.selectedItem() ? this.selectedItem().sEmail : '';
	
	if (this.hasOwnerContact())
	{
		sSearch = this.ownerContact().getEmailsString();
	}
...
CMailCache.prototype.setFolderCounts = function (iAccountId, sFullName, iCount, iUnseenCount, sUidNext, sHash)
{
	...
	if (oFolderList)
	{
		...
		if (oFolder)
		{
			bFolderHasChanges = oFolder.setRelevantInformation(sUidNext, sHash, iCount, iUnseenCount, true);
	...
	if (oResult !== false && oResult['@Object'] === 'Collection/MessageCollection')
	{
		...
		if (bCurrentList)
		{
			...
			if (bCurrentPage)
			{
				...
				this.checkMailStarted(false);
				this.setAutocheckmailTimer();
				Utils.log('Check mail started - parseMessageList', this.checkMailStarted());
			}
		}
...
CCalendarCache.prototype.onCalendarListResponse = function (oResponse, oRequest)
{
	if (oResponse && oResponse.Result)
	{
		...
		this.calendars(_.map(oResponse.Result, function (oCalendar) {
			return {'name': oCalendar.Name + ' <' + oCalendar.Owner + '>', 'id': oCalendar.Id,
				canEdit: oCalendar.Owner === iCurrentEmail ||
					(oCalendar.Access === Enums.CalendarAccess.Full || oCalendar.Access === Enums.CalendarAccess.Write)
			};
		}));
	}
