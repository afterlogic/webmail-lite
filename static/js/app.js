...
function CComposeViewModel()
{
	...
	this.selectedSender.subscribe(function (sSelectedSenderId) {
		var
			...
			oAccountName = oAccount.friendlyName(),
		...
		bRecipientsSameAsChiefAccount = _.any(aFetchersAndIdentities, function (oAddr) {
			return oAddr.email() === oAccountEmail && (oAddr.friendlyName ? oAddr.friendlyName() === oAccountName : oAddr.userName() === oAccountName);
		});
...
CCalendarViewModel.prototype.getDateFromCurrentView = function (sDateType)
{
	...
	;
	return (oView && oView[sDateType] && oView[sDateType]['unix']) ? oView[sDateType]['unix']() : 0;
};
...
{
	this.$datePicker.datepicker('setDate', this.$calendarGrid.fullCalendar('getDate').toDate());
	this.highlightWeekInDayPicker();
};
...
CCalendarViewModel.prototype.getParamsFromEventData = function (oEventData)
{
	return {
		...
		startTimestamp: oEventData.start.unix(),
		endTimestamp: oEventData.end ? oEventData.end.unix() : oEventData.end.unix(),
