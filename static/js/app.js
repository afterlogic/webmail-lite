													...
													oFetcherOrIdentity, sText, sDraftUid)
{
	...
	switch (sReplyType)
	{
		...
		case Enums.ReplyType.Resend:
			...
			oReplyData.Subject = this.replySubjectAdd(Utils.i18n('COMPOSE/RESEND_PREFIX'), oMessage.subject());
...
FolderCreatePopup.prototype.onShow = function (oFolderList, sName)
{
	if(sName)
	{
...
function CFolderModel()
{
	...
	this.subfoldersMessagesCountToShow = ko.observable({});
	this.subfolders = ko.observableArray([]);
	this.subfolders.subscribe(function (aSubfolders) {
		this.subfoldersMessagesCountToShow({});
		this.subscribeRecursively(aSubfolders);
	}, this);
	this.messageCountToShow = ko.computed(function () {
//		console.log(this.subfoldersMessagesCountToShow());
		if (this.expanded())
		{
		...
		{
			return (_.reduce(this.subfoldersMessagesCountToShow(), function(memo, num){return memo + num; }, 0)) + ((this.showUnseenMessages()) ? this.unseenMessageCount() : this.messageCount());
		}
	}, this);
...
CFolderModel.prototype.subscribeRecursively = function (aSubfolders)
{
	_.each(aSubfolders, function(oFolder, iKey) {
		oFolder.messageCountToShow.subscribe(function (iMessage) {
			this.subfoldersMessagesCountToShow()[oFolder.fullName()] = oFolder.unseenMessageCount();
			this.subfoldersMessagesCountToShow.valueHasMutated();

			if (oFolder.subfolders().length)
			{
				this.subscribeRecursively(oFolder.subfolders());
			}
		}, this);
	}, this);
};
...
CMailViewModel.prototype.onApplyBindings = function ()
{
	...
	$(document).on('keyup', function(ev) {
		...
		{
			bFirstMessageSelected = oFirstMessage.selected();
