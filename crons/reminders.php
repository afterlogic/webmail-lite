...
<div style="padding: 10px; font-size: 12px; text-align: center;">
	...
	private function getSubject($oAccount, $sEventStart, $iEventStartTS, $sEventName, $sDate, $iNowTS)
	{
		$sSubject = '';
	...
	public function Execute()
	{
		...
		if (file_exists($this->sCurRunFilePath))
		{
			...
			{
				$oStartDT_UTC = new \DateTime();
				$oStartDT_UTC->setTimestamp($sCurRunFileTS);
				$oStartDT_UTC->setTimezone($oTimeZoneUTC);
			}
		}
							...
							foreach ($aSubEvents as $mKey => $aEvent)
							{
								if ($mKey !== 'vcal')
								{
									...
									if ($oCalendar)
									{
										...
										$sSubject = $this->getSubject($oAccount, $sEventStart, $iEventStartTS, $sEventName, $sDate, $iNowTS_UTC);
