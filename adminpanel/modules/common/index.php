	...
	*/
	...
	protected function initTableList($sTab, ap_Screen &$oScreen)
	{
		if (AP_TAB_DOMAINS === $sTab)
		{
			...
			$iAllCount = $this->oDomainsApi->GetDomainCount($sSearchDesc, $iRealmId);
			$oScreen->EnableSearch(($iAllCount > 1) || $sSearchDesc);

			$bAddDefaultDomain = false;
			if (0 === $iRealmId && $this->oAdminPanel->HasAccessDomain(0))
			{
				$iAllCount++;
			...
			$aDomainsList = $this->oDomainsApi->GetDomainsList($oScreen->GetPage(),
				...
				$oScreen->GetOrderBy(), $oScreen->GetOrderType(), $sSearchDesc, $iRealmId);
		...
		if (AP_TAB_DOMAINS === $sTab)
		{
			switch ($sMainAction)
			{
				case 'edit':
					...
					$iRealmId = $this->oAdminPanel->RealmId();
					...
					if ($this->oAdminPanel->HasAccessDomain($iDomainId))
					{
						...
						if (!$oDomain && null !== $iDomainId)
						{
							...
							if ($oDomain && (0 === $iRealmId || $iRealmId === $oDomain->IdRealm))
							{
					...
					{
						$sRealmAddString = '';
						if (0 < $oDomain->IdRealm && $this->oAdminPanel->RType() && 0 === $this->oAdminPanel->RealmId())
						{
							$sRealmAddString = $this->oRealmsApi->GetRealmLoginById($oDomain->IdRealm);
							if (!empty($sRealmAddString))
							{
								$sRealmAddString = ' ('.$sRealmAddString.')';
							}
						}

						$oScreen->Data->SetValue('strDomainName', $oDomain->Name.$sRealmAddString);
	...
	*/
	...
	public function AuthCheckSet()
	{
		...
		if (null !== $mType && md5(CSession::Id().AP_VERSION.__FILE__) === CSession::Get(AP_SESS_AUTH, null))
		{
			...
			if (AP_SESS_AUTH_TYPE_REALM === $this->oAdminPanel->AuthType())
			{
				$iRealmId = (int) CSession::Get(AP_SESS_AUTH_REALM_ID, 0);
				if (0 < $iRealmId)
				{
					$this->setRealmAccessId($iRealmId);
				}
			}
		}
	}
	...
	public function AuthLogin($sLogin, $sPassword)
	{
		...
		{
			$iRealmId = $this->oAdminPanel->CallModuleFunction('CProModule',
				'GetRealmIdByLoginPassword', array($sLogin, $sPassword));

			if (0 < $iRealmId)
			{
				$this->setAdminAccessType(AP_SESS_AUTH_TYPE_REALM);
				$this->setRealmAccessId($iRealmId);
				return true;
			}
		}
	...
	protected function setAdminAccessType($iAccessType = AP_SESS_AUTH_TYPE_NONE)
	{
		...
		if (in_array((int) $iAccessType, array(AP_SESS_AUTH_TYPE_REALM,
	 ...
	 * @param int $iRealmId
	 */
	protected function setRealmAccessId($iRealmId)
	{
		if (0 < $iRealmId)
		{
			...
			if ($oRealm)
			{
				$this->oAdminPanel->SetRealmTabsInfo($oRealm);
			}
		}

		CSession::Set(AP_SESS_AUTH_REALM_ID, $iRealmId);
		$this->oAdminPanel->SetAuthRealmId($iRealmId);
	}
