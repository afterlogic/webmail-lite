...
class CCompatibilityStep extends AInstallerStep
{
	...
	function Validate()
	{
		...
		$this->aCompatibility['php.version.valid53'] = (int) (version_compare($this->aCompatibility['php.version'], '5.3.0') > -1);
		$this->aCompatibility['php.version.valid52'] = (int) (version_compare($this->aCompatibility['php.version'], '5.2.3') > -1);
		...
		$this->aCompatibility['compatibility'] = (int)
			$this->aCompatibility['php.version.valid52'] &&
	...
	{
		$bLite = @file_exists(WM_INSTALLER_PATH.'../libraries/afterlogic/common/lite.php');
		return array(
			'PhpVersion' => ($this->aCompatibility['php.version.valid53'])
				? $this->getSuccessHtmlValue('OK')
				: (($this->aCompatibility['php.version.valid52'])
					? $this->getWarningHtmlValue('PHP version is '.$this->aCompatibility['php.version'].'.<br /><br />'.
($bLite
	? 'WebMail Lite can work with PHP 5.2 but PHP 5.3 or higher is recommended.'
	: 'Calendar and mobile sync will be disabled as they require PHP 5.3.0 or higher.'),
'', false)
...
<a href="http://www.afterlogic.com/support/faq-webmail-pro-php#3.1" target="_blank">FAQ</a>.'),
	...
	function getWarningHtmlValue($sVarning, $sVarningHelp = '', $bNotDetected = true)
	{
		$sResult = $bNotDetected
			? '<span class="state_warning"><img src="./images/alarm.png"> Not detected. <br />'.$sVarning.'</span>'
			: '<span class="state_warning"><img src="./images/alarm.png"> '.$sVarning.'</span>';
